...
int findMinProcess(Process ProcessArray[], int NumberOfProcess, int Now) {
    ...
    for (i = 0; i < NumberOfProcess; i++) {
        if (ProcessArray[i].entranceTime <= Now) {
            if (ShortestProcessTime > ProcessArray[i].ProcessTime && ProcessArray[i].ProcessTime != 0) {
                indexOfProcess = i;
    ...
    if (EnteredProcesses < NumberOfProcess && indexOfProcess==-1)
        return -2;
...
int main() {
    ...
    while (true) {
        ...
        for(int i=0;i<SelectedProcess->ProcessTime;i++) {
            TimeCounter += 1;
            SelectedProcess->ProcessTime--;
            if(SelectedProcess->ProcessTime==0){
                NewSelection=true;
                SelectedProcess->exit=TimeCounter;
                break;
            }

            if (findMinProcess(ProcessArray, NumberOfProcess, TimeCounter) != indexOfProcess) {
                NewSelection = true;
                break;
            }
        }
